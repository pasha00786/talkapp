...
const setDate = () => {
  ...
  updateScrollbar();
    ...
    $('#online').text('Online');
     ...
     lobby.emit( 'madamjoin', {
            ashu:'ashu'            
        });
       lobby.on('ashujoin', (data) => {   
        $('#online').text('Online');
        lobby.emit( 'madamjoin', {
            ashu:'ashu'  
            })          
...
$(".message-input").keyup(function(){
    msg = $('.message-input').val();
  if (!$.trim(msg) == '' && count == 0) {
    lobby.emit( 'madamtyping', {
            ashu:'ashu'})
    count++; 
  }
  if($.trim(msg) == '' && count == 1)
    {lobby.emit( 'madamstoptyping', {
            ashu:'ashu'})
        count--;
      }
});
...
lobby.on('ashutyping', (payload) => {
    $('<div class="message loading new"><figure class="avatar"><img src="https://s3-us-west-2.amazonaws.com/s.cdpn.io/156381/profile/profile-80.jpg" /></figure><span></span></div>').appendTo($('.mCSB_container'));
    updateScrollbar();
});
lobby.on('ashustoptyping', (payload) => {
